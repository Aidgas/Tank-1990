...
void Player::update(Uint32 dt)
{
    ...
    {
        if( testFlag(TSF_LIFE) && idle_tank_play_channel != -1 && move_tank_play_channel != -1 )
        {
            if(    key_state[player_keys.up]
                || key_state[player_keys.down]
                || key_state[player_keys.left]
                || key_state[player_keys.right] )
            {
                Mix_Pause(idle_tank_play_channel);
                Mix_Resume(move_tank_play_channel);
            }
            else
            {
                Mix_Pause(move_tank_play_channel);
