    ...
    if(ev->type == SDL_KEYDOWN)
    {
        switch(ev->key.keysym.sym)
        {
            ...
            case SDLK_RETURN:
                ...
                if( m_pause )
                {
                    printf("sound_pause --> %d\n", Mix_PlayChannel(-1, sound_pause, 0));
                    
                    for(auto player : m_players)
                    {
                        player->stop_sound_tank();
                    }
                }
                else
                {
...
void Game::play_sound_game_over()
{
    Mix_PlayChannel(-1, destroy_player, 0);
}

void Game::free_sounds()
{
    if( sound_pause != NULL )
    {
    ...
    if( bullet_1 != NULL )
    {
        Mix_FreeChunk(bullet_1);
        bullet_1 = NULL;
    }
    
    if( bullet_2 != NULL )
    {
        Mix_FreeChunk(bullet_2);
        bullet_2 = NULL;
    }
    ...
    if( show_new_bonus != NULL )
    {
        Mix_FreeChunk(show_new_bonus);
        show_new_bonus = NULL;
    }
    
    if( add_new_tanklife != NULL )
    {
        Mix_FreeChunk(add_new_tanklife);
        add_new_tanklife = NULL;
    }
    ...
    if( take_bonus != NULL )
    {
        Mix_FreeChunk(take_bonus);
        take_bonus = NULL;
    }
    
    if( score_end != NULL )
    {
        Mix_FreeChunk(score_end);
        score_end = NULL;
    }
...
{
    std::string path = AppConfig::sounds_path + "pause.wav";
    
    sound_pause = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), sound_pause == NULL? 0 : 1);
    
    Mix_VolumeChunk(sound_pause, 55);
    
    path = AppConfig::sounds_path + "bullet_1.wav";
    
    bullet_1 = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), bullet_1 == NULL? 0 : 1);
    
    Mix_VolumeChunk(bullet_1, 55);
    
    path = AppConfig::sounds_path + "bullet_2.wav";
    
    bullet_2 = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), bullet_2 == NULL? 0 : 1);
    
    Mix_VolumeChunk(bullet_2, 55);
    ...
    Mix_VolumeChunk(bullet_3, 65);
    
    path = AppConfig::sounds_path + "show_new_bonus.wav";
    
    show_new_bonus = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), show_new_bonus == NULL? 0 : 1);
    
    Mix_VolumeChunk(show_new_bonus, 65);
    
    path = AppConfig::sounds_path + "add_new_tanklife.wav";
    
    add_new_tanklife = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), add_new_tanklife == NULL? 0 : 1);
    
    Mix_VolumeChunk(add_new_tanklife, 65);
    
    path = AppConfig::sounds_path + "destroy_bot.wav";
    
    destroy_bot = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), destroy_bot == NULL? 0 : 1);
    
    Mix_VolumeChunk(destroy_bot, 70);
    ...
    Mix_VolumeChunk(destroy_bot, 80);
    
    path = AppConfig::sounds_path + "take_bonus.wav";
    
    take_bonus = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), take_bonus == NULL? 0 : 1);
    
    Mix_VolumeChunk(take_bonus, 65);
    
    path = AppConfig::sounds_path + "score_end.wav";
    
    score_end = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), score_end == NULL? 0 : 1);
    
    Mix_VolumeChunk(score_end, 65);
    
    path = AppConfig::sounds_path + "start_game.wav";
    
    start_game = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), start_game == NULL? 0 : 1);
    
    Mix_VolumeChunk(start_game, 65);
    
    
    path = AppConfig::sounds_path + "game_over.wav";
    
    game_over = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), game_over == NULL? 0 : 1);
    
    Mix_VolumeChunk(game_over, 65);
    ...
    for(int i = row_start; i <= row_end; i++)
        for(int j = column_start; j <= column_end; j++)
        {
            ...
            if(intersect_rect.w > 0 && intersect_rect.h > 0)
            {
                ...
                if( ! bullet->is_boot )
                {
                    printf("bullet_1 --> %d\n", Mix_PlayChannel(-1, bullet_1, 0));
                }
                
                bullet->destroy();
                //bullet->play_bullet_1();
            }
        }
    ...
    for(auto bullet : player->bullets)
    {
        if(!bullet->to_erase && !bullet->collide)
        {
            ...
            if(intersect_rect.w > 0 && intersect_rect.h > 0)
            {
                ...
                {
                    m_enemy_to_kill--;
                    enemy->play_sound_destroy_bot();
                }
                else
                {
                    printf("bullet_3 --> %d\n", Mix_PlayChannel(-1, bullet_3, 0));
                }
...
void Game::checkCollisionPlayerWithBonus(Player *player, Bonus *bonus)
{
    ...
    if(intersect_rect.w > 0 && intersect_rect.h > 0)
    {
        ...
        {
            player->lives_count++;
            printf("add_new_tanklife --> %d\n", Mix_PlayChannel(-1, add_new_tanklife, 0));
        }
        else if(bonus->type == ST_BONUS_STAR)
        {
            player->changeStarCountBy(1);
            
            printf("score_end --> %d\n", Mix_PlayChannel(-1, score_end, 0));
        }
...
void Game::generateBonus()
{
    ...
    printf("show_new_bonus --> %d\n", Mix_PlayChannel(-1, show_new_bonus, 0));
}
