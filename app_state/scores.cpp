...
void Scores::init_sounds()
{
    std::string path = AppConfig::sounds_path + "score_end.wav";
    
    score_end = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), score_end == NULL? 0 : 1);
    
    Mix_VolumeChunk(score_end, 70);
    
    
    path = AppConfig::sounds_path + "beep_1.wav";
    
    beep_1 = Mix_LoadWAV(path.c_str());
    printf("load sound '%s' --> %d\n", path.c_str(), beep_1 == NULL? 0 : 1);
    
    Mix_VolumeChunk(beep_1, 70);
}

void Scores::free_sounds()
{
    if( score_end != NULL )
    {
...
{
    if(    m_show_time > AppConfig::score_show_time - 700 
        && score_end != NULL 
        && ! score_end_played)
    {
        score_end_played = true;
